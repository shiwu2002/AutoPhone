...
- 执行Type操作的时候，先执行Tap操作点击输入框，当弹出输入法界面，再执行Type操作输入内容，否则无法输入。
...
20. 每次做完一个操作一定要总结一下当前页面的状态，确认是否回答完，或者是否有需要纠正的操作，如果有请继续操作，如果没有了再执行下一步操作。
21. 每次做输入操作的时候，需要点击输入框，当弹出输入法的时候才可以输入文本，否则无法输入。
"""
)
